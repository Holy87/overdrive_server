...
{
    // restituisce il resultset, non l'oggetto
    public static function get_messages_obj(string $board_id): array {
        $board_id = safe_string($board_id);
        $query = get_connection()->prepare('select * from messages where sphere_id = :board');
    ...
    public static function get_message(int $message_id): BoardMessage {
        ...
        $query = get_connection()->prepare('select * from messages where message_id = :id');
...
where sphere_id = :board';
        $board_id = safe_string($board_id);
        $query = get_connection()->prepare($query_str);
        $query->bindParam(':board', $board_id);
        $query->execute();
        return $query->fetchAll(PDO::FETCH_ASSOC);
    }
    ...
    public static function create_message(string $game_id, string $sphere_id, string $message, string $reply_to = null) {
        $query = get_connection()->prepare('insert into messages (game_id, sphere_id, message, reply_to) values (:game, :sphere, :message :reply)');
