...
class PlayerRepository {
    ...
    public static function get_player_from_game(string $game_id): ?Player {
        return self::get_player_from('game_id', safe_string($game_id));
    }

    public static function get_player_from_name(string $name): ?Player {
        return self::get_player_from('player_name', safe_string($name));
    }

    public static function get_player_from(string $column, $key): ?Player {
        $query = get_connection()->prepare("select * from players where $column = :key");
    ...
    public static function check_name_exist(string $name): bool {
        $name = safe_string($name);
        $query = get_connection()->prepare('SELECT * FROM players where player_name = :name');
        $query->bindParam(':name', $name);
        $query->execute();
    ...
    public static function check_game_id_exist(string $game_id): bool {
        $name = safe_string($game_id);
        $query = get_connection()->prepare('SELECT * FROM players where game_id = :game_id');
        $query->bindParam(':game_id', $game_id);
        $query->execute();
        return $query->rowCount() > 0;
    }
